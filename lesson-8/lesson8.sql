...
/*Task 2*/
WITH AllYears AS (
    -- Generate a list of years from 1975 to the current year
...
MissingYears AS (
    -- Find years where no employee was hired
    SELECT Year
    FROM AllYears
    WHERE Year NOT IN (SELECT DISTINCT YEAR(HIRE_DATE) FROM EMPLOYEES_N)
),
GroupedYears AS (
    -- Create a grouping key using row difference trick
    SELECT 
        Year,
